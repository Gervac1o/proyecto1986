...
public interface ListaDocsRepository extends JpaRepository<ListaDocs,Integer> {
	...
	@Query(
            ...
            nativeQuery = false)
    List<ListaDocs> findDictamen(@Param("idAlumno") Integer idAlumno);
	
	@Query(
            value = "SELECT s FROM ListaDocs s WHERE s.idAlumno = :idAlumno AND s.idTramite = 2",
            nativeQuery = false)
    List<ListaDocs> findLiberacion(@Param("idAlumno") Integer idAlumno);
	
	@Query(
            value = "SELECT s FROM ListaDocs s WHERE s.idAlumno = :idAlumno AND s.idTramite = 3",
            nativeQuery = false)
    List<ListaDocs> findBaja(@Param("idAlumno") Integer idAlumno);
	
	@Query(
            value = "SELECT s FROM ListaDocs s WHERE s.idAlumno = :idAlumno AND s.idTramite = 4",
            nativeQuery = false)
    List<ListaDocs> findServicio(@Param("idAlumno") Integer idAlumno);
